...
int main() {
  ...
  for (char c : res1) std::cout << c;
  std::cout << '\n';

  auto res2 = getPerm2(tree, 2);
  std::cout << "Перестановка №2: ";
  for (char c : res2) std::cout << c;
  std::cout << '\n';
  ...
  for (int n : sizes) {
    ...
    double time_all = std::chrono::duration<double, std::milli>(end_all - start_all).count();
    double time_p1 = std::chrono::duration<double, std::milli>(end_p1 - end_all).count();
    double time_p2 = std::chrono::duration<double, std::milli>(end_p2 - end_p1).count();

    file << n << ", " << time_all << ", " << time_p1 << ", " << time_p2 << "\n";

    std::cout << "n=" << n << ": getAll=" << time_all
              << "ms, getPerm1=" << time_p1 << "ms, getPerm2=" << time_p2 << "ms\n";
  }
